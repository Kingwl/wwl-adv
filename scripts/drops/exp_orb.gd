...
func _ready() -> void:
	...
	for i in range(5):
		var texture_path := "res://assets/art/effects/by_type/drop_exp_orb/exp_orb_%02d.png" % (i + 1)
		...
		if texture:
			...
			push_warning("ExpOrb: failed to load texture %s" % texture_path)
