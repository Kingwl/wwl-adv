...
- 行为：持续追踪玩家，碰撞后造成伤害
- 状态：支持 `slow` 和 `stun`
- 生成：从玩家周围 450-700 像素环形区域随机生成
- 难度曲线：生成间隔随时间缩短；HP、速度、伤害按时间缩放
- 资源化：`EnemyData` 已定义，生成器已支持读取 `resources/enemies/`，当前尚未配置敌人 `.tres`

### 武器

当前有 17 种武器，开局自带基础利刃，其余通过升级解锁。
...
升级候选会过滤已满级武器，并保证同一轮不会重复出现同一个武器 ID。升级面板支持刷新全部候选卡片，也支持跳过本次选择直接继续游戏。

### UI

- 主菜单：开始游戏
- HUD：时间、击杀、金币、HP、EXP、等级、武器栏、属性按钮
- 升级选择：三张卡片，显示图标、类型和武器分类
- 暂停菜单：展示当前武器槽位并可返回主菜单
- StatsPanel：查看角色基础状态和已持有武器详情
- GameOver：展示时间、击杀和本局武器
